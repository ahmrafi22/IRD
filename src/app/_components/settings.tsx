...
export function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  const SettingsContent = () => {
    ...
    const settingsItems = [
      { id: "language", svg: "lang.svg", label: "Language Settings" },
      { id: "general", svg: "cards.svg", label: "General Settings" },
      { id: "font", svg: "sett.svg", label: "Font Settings" },
      { id: "appearance", svg: "sett.svg", label: "Appearance Settings" },
    ];
          ...
          return (
            <div
              ...
              className={`relative flex items-center space-x-3 p-3 rounded-2xl cursor-pointer transition-colors duration-200 ${
                isActive
                  ? "bg-[#f0f9f4] text-[#1fa45b]"
                  : "text-[#393939] hover:bg-[#f5f5f5]"
              }`}
              onClick={() => setActiveItem(item.id)}
            >
              {isActive && (
                <div className="absolute left-0 top-0 bottom-0 w-[5px] bg-[#1fa45b] rounded-l-full"></div>
              )}
              <Image
                src={`/svgs/${item.svg}`}
                alt={item.label}
    ...
    <>
      ...
      >
        <div className="bg-white p-4 flex items-center justify-between z-10">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-2xl hover:bg-gray-100 transition-colors flex items-center justify-center"
          >
      ...
      {/* Desktop Panel */}
      <div
        className="hidden xl:flex xl:fixed xl:right-0 mt-1 bg-white border h-screen border-[#e2e2e2] flex-col overflow-hidden"
        style={{ width: "330px" }}
      >
        <h2 className="text-xl mt-6 text-center font-semibold text-[#393939]">
          Settings
        </h2>
